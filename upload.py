...
options.add_argument("user-data-dir=C:\\Users\\Dmd27\\AppData\\Local\\Google\\Chrome Beta\\User Data\\")
...
if(int(answer) == 1):
    ...
    for i in range(int(howmany)):
        bot = webdriver.Chrome('C:\\wd\\chromedriver.exe',chrome_options=options)
        try:
            
            bot.get("https://studio.youtube.com")
            time.sleep(6)
            upload_button = bot.find_element(By.XPATH, '//*[@id="upload-button"]')
            upload_button.click()
            time.sleep(5)

            file_input = bot.find_element(By.XPATH, '//*[@id="select-files-button"]')
            simp_path = f'C:\\Users\Dmd27\\Desktop\\Youtube Shorts Bot\\videos{nameofvid}'
